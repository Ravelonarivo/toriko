...
export const getLocationBySlugAndTownName = async (locationSlug, townName) => {
	try {
		return await db('location')
			...
			.select('location.id' ,'location.name', 'location.slug', 'location.latitude', 'location.longitude', 'location.address', 'location.phone', 'location.delivery', 'location.logo', 'location_type.name as type', 'speciality.name as speciality', 'district.name as district');
	} catch (error) {
		console.log(error);
	}
};
